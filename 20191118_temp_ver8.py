...
def main(): #=====================================
    ...
    # 直線上のヒストグラムのピークを求める
    """
    maxid = signal.argrelmax(height, order=100)
    
    Fig = plt.figure(figsize=(6, 6))
    Map1 = Fig.add_subplot(111)    
    Map1.plot(left, height)
    
    Map1.scatter(left[maxid[0]], height[maxid[0]])
    for i in range(len(maxid[0])):
        Map1.text(left[maxid[0][i]], height[maxid[0][i]], 'PEAK!!!')
    ...
    """        
    plt.bar(left, height)
    #plt.savefig("./output/patern_graph.png")
    plt.show()
    """
