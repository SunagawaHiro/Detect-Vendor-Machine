...
def main(): #=====================================
    ...
    # 2点間の直線のヒストグラムを求める
    P1 = np.array([0,1898])
    P2 = np.array([4032,1976])
    #itbuffer = createLineIterator(P1, P2, gray_input)
    #left = itbuffer[0:len(itbuffer), 0]
    #height = itbuffer[0:len(itbuffer),2]

    """
    # matchTemplateの挙動について把握する
    gray_similar_max_img = cv2.cvtColor(similar_max_img, cv2.COLOR_RGB2GRAY)       
    print(temp.shape)
    print(gray_similar_max_img.shape)
    res = cv2.matchTemplate(gray_similar_max_img,temp,cv2.TM_CCOEFF_NORMED)
    print(res)
    """
    ...
    line = makeLinearEquation(0, 1898, 4032, 1976) # (x1, y1, x2, y2)
    m = line["m"]
    n = line["n"]
    ...
    print(len(line_crd))
    new_line_crd = line_crd[wt:w_input-wt]
    print(len(new_line_crd))
    ...
    top_f = new_line_crd[0][1] - h_move
    bottom_f = new_line_crd[0][1] + h_move
    left_f = new_line_crd[0][0] - w_move
    right_f = new_line_crd[0][0] + w_move

    print("最初:top,bottom,left,right",top_f,bottom_f,left_f,right_f)
    crop_first = gray_input[ top_f : bottom_f, left_f : right_f] # img[top : bottom, left : right]
    cv2.imwrite("./output/crop_first.png", crop_first)
    
    print(new_line_crd[3505])

    top_e = new_line_crd[3505][1] - h_move
    bottom_e = new_line_crd[3505][1] + h_move
    left_e = new_line_crd[3505][0] - w_move
    right_e = new_line_crd[3505][0] + w_move

    print("最後:top,bottom,left,right",top_e,bottom_e,left_e,right_e)
    crop_end = gray_input[ top_e : bottom_e, left_e : right_e] # img[top : bottom, left : right]
    cv2.imwrite("./output/crop_end.png", crop_end)
    """
    ...
    left = list(range(len(ruijido_list)))
    height = ruijido_list
    plt.bar(left, height)
    plt.show()
