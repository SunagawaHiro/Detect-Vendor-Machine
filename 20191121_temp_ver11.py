...
def main(): #=====================================
    ...
    print("md.data_3511の長さ:",len(md.data_3511))
    left = list(range(w_move,w_input-w_move,1))
    print("新しいグラフ横幅の大きさです",len(left))
    #print(md.data_3511)
    plt.bar(left, md.data_3511)
    plt.show()
    
    # ピーク検出を行う
    data_3511_numpy = np.array(md.data_3511)
    peak = wt
    maxid = signal.argrelmax(data_3511_numpy, order=peak)
    print("ピーク位置:",maxid[0])
    for i in maxid[0]:   
        print(md.data_3511[i])

    print(md.data_3511[0])
    print(md.data_3511[385])
    
    Fig = plt.figure()
    Map1 = Fig.add_subplot(111)    
    Map1.plot(left, data_3511_numpy)
    for i in range(len(maxid[0])):
        Map1.text(left[maxid[0][i]], data_3511_numpy[maxid[0][i]], 'PEAK!')    
