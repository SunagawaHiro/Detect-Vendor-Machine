...
def color_hist_compare(temp_img, crop_img):
    ...
    cv2.imwrite("./output/4245_jpg/temp.png",temp_img)
    crop_img = cv2.resize(crop_img, IMG_SIZE)
    cv2.imwrite("./output/4245_jpg/hikaku_taisyou.png",crop_img)
...
def main(): #=====================================
    ...
    # rgb画像に域値以上の結果を描画する
    cv2.imwrite("./output/4245_jpg/ikiti_ijou_{}_{}_{}.jpg".format(scale,FileName,threshold), img_input)
    ...
    cv2.imwrite("./output/4245_jpg/max_crop.png", similar_max_img)
    ...
    line = makeLinearEquation(0, 2010, 4032, 2040) # (x1, y1, x2, y2)
    m = line["m"]
    n = line["n"]
    ...
    plt.bar(left, md.data_4245)
    plt.savefig("./output/4245_jpg/4245_ruijido_graph.png")
    #plt.show()

    
    # ピーク検出を行う
    data_4245_numpy = np.array(md.data_4245)
    print("data_4245 の長さ:",len(md.data_4245))
    peak = wt
    maxid = signal.argrelmax(data_4245_numpy, order=peak)
    print("ピーク位置:",maxid[0])
    for i in maxid[0]:   
        print(md.data_4245[i])
    
    Fig = plt.figure()
    Map1 = Fig.add_subplot(111)    
    Map1.plot(left, data_4245_numpy)
    for i in range(len(maxid[0])):
        Map1.text(left[maxid[0][i]], data_4245_numpy[maxid[0][i]], 'PEAK!')    
