...
def main(): #=====================================
    ...
    # 各列の4点を通る回帰直線を引く準備
    print("P1:",P1)   # [[0.9410657, 3427, 817], [0.92706907, 2555, 823], [0.92609495, 2845, 820], [0.9235415, 3135, 819]]
    x_points = []
    y_points = []    
    for i in P1:
    ...
    # 回帰直線を計算し,1次関数を求める(http://ailaby.com/least_square/)
    print("1次関数m,n = ",np.polyfit(x_points, y_points, 1))
    func = np.poly1d(np.polyfit(x_points, y_points, 1))

    # 得られた1次関数からスタートとゴール位置を把握
    print("w_input = ",w_input)
    start = int(func(0))
    goal = int(func(w_input))
    print("start = ",start)
    print("goal = ",goal)
    print()
    ...
    ruijido_list = np.array(md.new_data_3841)
    print("len(ruijido_list)",len(ruijido_list))
    
    plt.bar(left, ruijido_list)
    plt.savefig("./output/{}_jpg/20191127/{}_ruijido_graph.png".format(FileName,FileName))
    plt.show()
    print("OK")
