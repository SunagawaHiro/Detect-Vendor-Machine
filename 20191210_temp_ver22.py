...
def main(): #=====================================
    ...
    # rgb画像に域値以上の結果を描画する
    cv2.imwrite("./output/{}_jpg/20191210/ikiti_ijou_{}_{}_{}.jpg".format(FileName,max_btn_nums_scl[1],FileName,threshold), img_input)
    ...
    plt.savefig("./output/3841_jpg/20191210/num_button_graph/button_num_detect_add_hmove.png")    
    ...
    for i in maxid[0]:
        similar_to_find_max.append(md.data_3841[i])
        a = [i+w_move, md.data_3841[i]]
    ...
    for i in crd_similar:
        if i[1] >= max(similar_to_find_max)*0.8:  # 検出したピークの最大類似度の80%以上のピークのみを抽出する
    ...
    # ボトルの切り出しを行う ================================= この処理も後ほど関数化していく

    # 隣り合うボタン同士の距離を比較する
    # ピークとピークの距離を求め, その平均を求める

    # 検出したピーク(おそらくボタン)crd だけを取り出して新しい配列に保存する
    only_peak_crd = []
    ...
    print()
    print("スケール変換後のテンプレの高さ・幅",ht,wt)
    crop_btl_h = int(wt * (210/44))
    crop_btl_w = int(wt * (67/44))
    ...
    for i in only_peak_crd:
        ...
        top = int((-0.006538*i + 896) - (h_move) - (crop_btl_h))     # func の関数の中身 今は応急処置: -0.006538 x + 896
        if top < 0:      # クロップする際に画面の端を超えてしまった場合は端っこまでをクロップすることにする
            top = 0
        bottom = int((-0.006538*i + 896) - (h_move))
        left = i-int(ave_between_peaks/2)
        right = i+int(ave_between_peaks/2)                
        img_crop = img_to_crop[ top: bottom, left: right]                
        cv2.imwrite("./output/{}_jpg/20191210/drink_crop_img/crop_{}.jpg".format(FileName,i), img_crop)        
