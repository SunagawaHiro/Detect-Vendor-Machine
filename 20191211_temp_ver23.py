...
def crop_and_save_bottle_fig(new_crd_similar,temp,FileName,h_move,func):  # 関数化2
    # ボトルの切り出しを行う ================================= 
    # 隣り合うボタン同士の距離を比較する
    ...
    for i in only_peak_crd:
        ...
        cv2.imwrite("./output/{}_jpg/20191211/drink_crop_img/crop_{}.jpg".format(FileName,i), img_crop)        
...
def main(): #=====================================
    ...
    for i in b_all:
        ...
        #print("len(ruijido_list) = ",len(ruijido_list))
        th = 0.8 # 類似度を抽出するときに、(最大類似度*th)の形で指定している。その際にこれを使用する
        ruijido_list = md.new_data_3841
        new_crd_similar = get_peak_of_ruijido_list(ruijido_list,th,wt,w_move)
        crop_and_save_bottle_fig(new_crd_similar,temp,FileName,h_move,func) # この関数は2つの処理を1つの関数内で行なっている。もっと分割できるはず
        exit()
