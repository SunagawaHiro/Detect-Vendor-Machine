...
def crop_and_save_bottle_fig(new_crd_similar,temp,FileName,h_move,func,count):  # 関数化2
    ...
    for i in only_peak_crd:
        ...
        cv2.imwrite("./output/{}_jpg/20191212/drink_crop_img/crop_{}_{}.jpg".format(FileName,count,i), img_crop)        
...
def main(): #=====================================================================================
    ...
    print("res[1993][2714]: ",res[1993][2714]) # (x,y) = (2714, 1993) # res は[y][x]の順で指定してやらないといけない
    print()
    

    """
    # ここで座標を指定すると, 類似度を取り出せるのか確認する    
    ...
    print("loc = ",loc)
    print()
    #print("res[70][1221] = ",res[70][1221])
    print("res[1221-h_move][70-w_move] = ",res[1221-h_move][70-w_move])
    #print("res[4032]",res[300][4032])
    """    
        
    # rgb画像に域値以上の結果を描画する
    cv2.imwrite("./output/{}_jpg/20191212/ikiti_ijou_{}_{}_{}.jpg".format(FileName,max_btn_nums_scl[1],FileName,threshold), img_input)
    ...
    plt.savefig("./output/{}_jpg/20191212/num_button_graph/{}_midstep_peak_add_hmove.png".format(FileName, FileName))
    plt.close()
    ...
    for i in b_all:        
        ...
        print("最初の結果を使用して,ruijido_listを作成しました.\n")
        
        # 最初のマッチング結果を利用すれば, ここの関数が必要なくなるはず
