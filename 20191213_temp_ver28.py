...
def main(): #=====================================================================================    
    ...
    res_old = cv2.matchTemplate(gray_input,temp,cv2.TM_CCOEFF_NORMED)    
    threshold = 0.7
    loc = np.where( res_old >= threshold)
    for pt in zip(*loc[::-1]):
    ...
    # 類似度が最大の時の画像を抜き取り, new_tempディレクトリに保存する
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res_old)
    print("max_val:{}\nmax_loc:{}".format(max_val,max_loc))
    img_input = cv2.imread("../../vm_full_img/IMG_{}.jpg".format(FileName))
    new_temp = img_input[max_loc[1]:max_loc[1]+ht, max_loc[0]:max_loc[0]+wt]    # img[top : bottom, left : right]
    new_dir_path_recursive = './auto_output/{}/{}/max_similar_temp'.format(datetime.date.today(),FileName)
    os.makedirs(new_dir_path_recursive, exist_ok=True)
    cv2.imwrite("{}/max_similar_temp.jpg".format(new_dir_path_recursive), new_temp)


    # 新しいテンプレートで再度検出を行う
    new_temp = cv2.cvtColor(new_temp, cv2.COLOR_RGB2GRAY)
    res = cv2.matchTemplate(gray_input,new_temp,cv2.TM_CCOEFF_NORMED)
    threshold = 0.7    
    loc = np.where( res >= threshold)
    for pt in zip(*loc[::-1]):
        cv2.rectangle(img_input, pt, (pt[0] + wt, pt[1] + ht), (0,0,255), 2)
    print("新しいテンプレートでの検出個数: {}\n".format(len(loc[0])))
            
    # rgb画像に域値以上の結果を描画する
    new_dir_path_recursive = './auto_output/{}/{}/btn_detect'.format(datetime.date.today(),FileName)
    os.makedirs(new_dir_path_recursive, exist_ok=True)
    cv2.imwrite("{}/ikiti_ijou_newtemp_{}_{}_{}.jpg".format(new_dir_path_recursive,max_btn_nums_scl[1],FileName,threshold), img_input)
    ...
    os.makedirs(new_dir_path_recursive, exist_ok=True)
    plt.savefig("{}/peak_add_hmove.png".format(new_dir_path_recursive))
    plt.close()
