...
def crop_and_save_bottle_fig(new_crd_similar,temp,FileName,h_move,func,count):  # 関数化2
    ...
    for i in only_peak_crd:
        ...
        os.makedirs(new_dir_path_recursive, exist_ok=True)
        cv2.imwrite("{}/crop_{}_{}.jpg".format(new_dir_path_recursive,count,i), img_crop)        
...
def main(): #=====================================================================================    
    ...
    for i in maxid[0]: # ピーク位置: [1061 1833 2603]
        ...
        res_first = res[ res_top : res_bottom+1 ]         
        # 分割した中での最大類似度を求める
    ...
    # 各ボタン列から抜き出したボタンを使用して,テンプレマッチングした結果,得られた検出個数(縦軸),画像の縦幅(横軸)として, グラフを作成する
    # ピークも同時に描画する    
    # ピーク位置を取得
    tate_graph_update = np.array(tate_graph_update)
    maxid = signal.argrelmax(tate_graph_update, order=wt*3) # 最大値 # orderの決め方はとても雑に書いてある(wt*3).根拠は欲しいな
    print("tate_graph_updateのピーク位置: {}".format(maxid[0]))
    
    # グラフを保存する
    Fig = plt.figure()    
    Map1 = Fig.add_subplot(111)    
    Map1.plot(left_update, tate_graph_update)
    for i in range(len(maxid[0])):
        Map1.text(left_update[maxid[0][i]], tate_graph_update[maxid[0][i]], 'PEAK!')            
    new_dir_path_recursive = './auto_output/{}/{}/button_line_peak_graph'.format(datetime.date.today(),FileName)
    os.makedirs(new_dir_path_recursive, exist_ok=True)
    plt.savefig("{}/peak_sum_{}times.png".format(new_dir_path_recursive,count))
    plt.close()
    
    # 検出したボタン列のピークの範囲を元に類似度グラフ(縦軸類似度,横軸画像の横幅)を作成する
    print("maxid[0] = ", maxid[0])
    left_w = list(range(0, w_input-wt+1,1))    
    for i in range(len(maxid[0])):
        res_at = res_alls[i]
        peak_at = maxid[0][i]
        # ボタンの位置を抜き出す
        print("len(res_at) = ",len(res_at))
        # resを必要なところだけ切り出す
        res_top,res_bottom = peak_at-(2*h_move),peak_at+(2*h_move)
        print("peak_at,res_top,res_bottom",peak_at,res_top,res_bottom)        
        res_crop = res[ res_top : res_bottom+1 ]         
        print("len(res_crop) = ",len(res_crop))        
        # 合計を求める
        res_crop_sum = np.sum(res_crop,axis = 0)
        print("res_crop_sum = ",res_crop_sum)
        print("len(res_crop_sum) = {}\n".format(len(res_crop_sum)))
        # グラフを求める
        # ピーク位置を取得        
        max_res_id = signal.argrelmax(res_crop_sum, order=wt) 
        print("res_crop_sumのピーク位置: {}".format(max_res_id[0]))        
        # グラフを保存する
        Fig = plt.figure()    
        Map1 = Fig.add_subplot(111)
        print("len(left_w):",len(left_w))
        print("len(res_crop_sum):",len(res_crop_sum))        
        Map1.plot(left_w, res_crop_sum)
        for i in range(len(max_res_id[0])):
            Map1.text(left_w[max_res_id[0][i]], res_crop_sum[max_res_id[0][i]], 'PEAK!')            
        new_dir_path_recursive = './auto_output/{}/{}/drink_peak_graph'.format(datetime.date.today(),FileName)
        os.makedirs(new_dir_path_recursive, exist_ok=True)
        plt.savefig("{}/peak_{}.png".format(new_dir_path_recursive,i))
        plt.close()
        exit()
    ...
    for i in b_all:        
        ...
        for j in new_line_crd:            
            ...
            ruijido_list.append(res[y_crd][x_crd])        
